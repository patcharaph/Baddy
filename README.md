...
**กระดานคิวคือหน้าเดียวที่ได้ความกว้างเพิ่ม** — ตั้งแต่ `lg` (1024px) ขึ้นไป มันขอกรอบกว้าง
1180px (คอลัมน์ 952px) ด้วยการติด `data-board` ไว้ที่ตัวมันเอง แล้วเปลือกอ่านค่านั้นด้วย
`:has()` วิธีนี้ทำให้ **หน้าจอเป็นคนบอกว่าตัวเองอยากได้ความกว้าง ส่วนเปลือกยังเป็นคนกำหนด
ตัวเลข** และเปลือกไม่ต้องรู้จักชื่อ route ไหนเลย ต่ำกว่า `lg` การประกาศนี้ไม่มีผล เพราะกรอบที่
กว้างกว่าหน้าต่างไม่ใช่ความกว้าง และเบราว์เซอร์ที่เก่าเกินกว่าจะรู้จัก `:has()` ก็ได้คอลัมน์ 652px
เท่ากับทุกหน้าอยู่ดี

ในกรอบนั้นคอร์ทวางเรียงสองใบต่อแถว ได้ใบละ ~454px ซึ่ง**กว้างกว่าบนมือถือ** ไม่ใช่ถูกบีบให้
ไปแทรกข้างกัน 4 คอร์ทจึงเหลือ 2 แถว และคิวถัดไปที่อยู่ใต้มันขึ้นมาอยู่ในจอเดียวกันโดยไม่ต้อง
เลื่อน — ซึ่งคือทั้งหมดที่กระดานนี้ต้องการ: เห็น "คอร์ทไหนว่าง" กับ "ใครรออยู่" พร้อมกัน
คิวก็แบ่งสองคอลัมน์ด้วยเหตุผลเดียวกับที่คอลัมน์หลักมีเพดาน — แถวเดียวยาว 920px คือชื่อชิดซ้าย
เวลารอชิดขวา แล้วว่างเปล่าตรงกลาง

**การ์ดคอร์ทว่างจะจัดข้อความไว้กลางใบเมื่ออยู่ข้างคอร์ทที่กำลังเล่น** เพราะแถวนั้นสูงเท่าใบที่สูง
กว่าอยู่แล้ว จะยืดหรือไม่ยืดก็ไม่ได้กินที่เพิ่ม บนมือถือไม่มีที่เหลือให้ยืด มันจึงยังเป็นบรรทัดเดียว
เท่าเดิม
